...
void CCheckBox::Draw(bool hover)
{
	...
	Render::GradientV(a.x, a.y, 13, 13, Color(23, 23, 23, 255), Color(28, 28, 28, 255));

	if (hover)
	{
		if (Checked)
		{
			Render::Text(a.x + 1, a.y, Color(0, 161, 255, 255), Render::Fonts::Menu, (L"\u2714"));
		}
		else
		{
...
{
	int red = Menu::Window.GUITab.RInner.GetValue();
	int green = Menu::Window.GUITab.GInner.GetValue();
	int blue = Menu::Window.GUITab.BInner.GetValue();

	POINT a = GetAbsolutePos();

	Render::Clear(a.x, a.y + 5, m_iWidth, 5, Color(40, 40, 40, 255));
	Render::Outline(a.x - 1, a.y + 4, m_iWidth + 2, 7, UI_COL_SHADOW);
	Render::GradientV(a.x, a.y + 5, m_iWidth, 3, Color(68, 68, 68, 255), Color(58, 58, 58, 255));

	float Ratio = Value / (Max - Min);
	float Location = Ratio*m_iWidth;

	//Render::Clear(a.x, a.y + 5, Location, 5, Color(151, 225, 3, 255)); /*Slider Color*/
	Render::GradientV(a.x, a.y + 5, Location, 5, Color(0, 161, 255, 255), Color(0, 161, 255, 240));
	...
	Render::Text(a.x + (Ratio / m_iWidth + Location) - txtSize.right / 2, a.y + 6.5, Color(255, 255, 255, 255), Render::Fonts::MenuBold, buffer);
}
...
void CKeyBind::Draw(bool hover)
{
	...
	Render::Clear(a.x, a.y, m_iWidth, m_iHeight, Color(35, 35, 35, 255));
	bool GoodKeyName = false;
	char NameBuffer[128];
	char* KeyName = "Not Bound";
	if (hover)
	{
		Render::GradientV(a.x + 2, a.y + 2, m_iWidth - 4, m_iHeight - 4, Color(50, 50, 50, 200), Color(60, 60, 60, 255));
	}
	if (IsGettingKey)
	{
		Render::GradientV(a.x + 2, a.y + 2, m_iWidth - 4, m_iHeight - 4, Color(50, 50, 50, 200), Color(60, 60, 60, 255));
...
{
	POINT a = GetAbsolutePos();
	Render::Outline(a.x, a.y, m_iWidth, 27, Color(0, 0, 0, 255));
	Render::GradientV(a.x + 2, a.y + 2, m_iWidth - 4, 23, Color(25, 25, 25, 255), Color(33, 33, 33, 255));
...
void CComboBox::Draw(bool hover)
{
	...
	Render::Clear(a.x, a.y, m_iWidth, 16, Color(35, 35, 35, 255));

	// Hover for the Top Box
	if (GUI.IsMouseInRegion(Region))
		Render::GradientV(a.x + 2, a.y + 2, m_iWidth - 4, 12, Color(50, 50, 50, 255), Color(60, 60, 60, 255));
	//Render::Clear(a.x + 2, a.y + 2, m_iWidth - 4, 12, Color(60, 60, 60, 255));
	...
	if (Items.size() > 0)
	{
		...
		if (IsOpen)
		{
			//Render::Clear(a.x, a.y + 17, m_iWidth, Items.size() * 16, Color(60, 60, 60, 255));
			Render::GradientV(a.x, a.y + 17, m_iWidth, Items.size() * 16, Color(50, 50, 50, 255), Color(60, 60, 60, 255));
			...
			for (int i = 0; i < Items.size(); i++)
			{
				...
				{
					Render::GradientV(a.x, a.y + 17 + i * 16, m_iWidth, 16, Color(50, 50, 50, 255), Color(60, 60, 60, 255));
