...
void CGUI::Draw()
{
	...
	if (ShouldDrawCursor)
	{
		...
		for (int i = 0; i < 4; i++)
			Render::Clear(Mouse.x + 2 + i, Mouse.y + 2 + i, 1, 14 - (i * 2), Color(0, 161, 255, 255));
		Render::Clear(Mouse.x + 6, Mouse.y + 6, 1, 8, Color(0, 161, 255, 255));
		Render::Clear(Mouse.x + 7, Mouse.y + 7, 1, 9, Color(0, 161, 255, 255));
		for (int i = 0; i < 4; i++)
			Render::Clear(Mouse.x + 8 + i, Mouse.y + 8 + i, 1, 4 - i, Color(0, 161, 255, 255));
		Render::Clear(Mouse.x + 8, Mouse.y + 14, 1, 4, Color(0, 161, 255, 255));
		Render::Clear(Mouse.x + 9, Mouse.y + 16, 1, 2, Color(0, 161, 255, 255));
	}
}
...
bool CGUI::DrawWindow(CWindow* window, int menu)
{
...

	{
		...
		Render::Clear(window->m_x + 8 - 90, window->m_y + 1 + 28 * 2, window->m_iWidth - 4 - 12 + 90, window->m_iHeight - 2 - 8 - 26, Color(17, 17, 17, MenuAlpha));
		...
		//Tab
		Render::Clear(window->m_x + 8 - 90, window->m_y + 1 + 28 * 2, 90, window->m_iHeight - 2 - 8 - 26, Color(12, 12, 12, MenuAlpha));

		Render::Outline(window->m_x + 8 - 90, window->m_y + 1 + 28 * 2, window->m_iWidth - 4 - 12 + 90, window->m_iHeight - 2 - 8 - 26, Color(48, 48, 48, 120));
		...
		//Render::GradientH(window->m_x + 8 - 90 + 1, window->m_y + 1 + 28 * 2 + 1, iProportion, 2, Color(137, 207, 240, MenuAlpha), Color(147, 112, 219, MenuAlpha));
		//Render::GradientH(window->m_x + 8 - 90 + 1 + iProportion, window->m_y + 1 + 28 * 2 + 1, iProportion, 2, Color(147, 112, 219, MenuAlpha), Color(245, 245, 5, MenuAlpha));
		//Render::GradientH(window->m_x + 8 - 90 + 1 + (iProportion * 2), window->m_y + 1 + 28 * 2 + 1, iProportion, 2, Color(245, 245, 5, MenuAlpha), Color(173, MenuAlpha, 47, MenuAlpha));
		...
		if (TabCount) // If there are some tabs
		{
			...
			for (int i = 0; i < TabCount; i++)
			{
				...
				if (window->SelectedTab == tab) {
					...
					Render::Clear(TabDrawArea.left, TabDrawArea.top, TabDrawArea.right, TabDrawArea.bottom, Color(17, 17, 17, MenuAlpha));
					...
					Render::GradientH(window->m_x - 80, window->m_y + 57, window->m_iWidth / 1.5, 2, Color(0, 161, MenuAlpha, MenuAlpha), Color(0, 161, MenuAlpha, MenuAlpha));
					Render::GradientH(window->m_x + window->m_iWidth / 2 - 10, window->m_y + 57, window->m_iWidth / 1.99 - 1.99, 2, Color(0, 161, MenuAlpha, MenuAlpha), Color(0, 161, MenuAlpha, MenuAlpha));
...
void CGUI::SaveWindowState(CWindow* window, std::string Filename)
{
	...
	{
		MessageBox(NULL, "Failed To Save Config File!", "c0caine", MB_OK);
	}
