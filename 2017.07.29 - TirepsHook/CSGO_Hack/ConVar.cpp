...
void ConVar::SetValue(float value)
{
	...
	return CallVFunc<OriginalFn>(this, 15)(this, value);
}
...
char* ConVar::GetName()
{
	...
	return CallVFunc<OriginalFn>(this, 5)(this);
}
...
SpoofedConvar::SpoofedConvar(const char* szCVar) {
	m_pOriginalCVar = Interface.CVar->FindVar(szCVar);
	Spoof();
}
...
SpoofedConvar::~SpoofedConvar() {
	if (IsSpoofed()) {
		...
		Interface.CVar->UnregisterConCommand(m_pDummyCVar);
		free(m_pDummyCVar);
...
void SpoofedConvar::Spoof() {
	if (!IsSpoofed() && m_pOriginalCVar) {
		...
		//Register it
		Interface.CVar->RegisterConCommand(m_pDummyCVar);
