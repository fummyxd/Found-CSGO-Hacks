...
namespace CAimbot
{
	...
	bool GetBestTarget(Vector3 myAng, bool allowSilent)
	{
		...
		for (int i = 0; i < Interface.Engine->GetMaxClients(); i++)
		{
			...
			if (pBaseEntity == pLocalEntity)
				continue;
			if (!U::IsVisible(pLocalEntity->GetEyePosition(), pBaseEntity->GetEyePosition(), pLocalEntity, pBaseEntity, Options::Aimbot::SmokeCheck))
				continue;
			if (!Options::Aimbot::Deathmatch && pBaseEntity->GetTeam() == pLocalEntity->GetTeam())
				continue;
		...
		if (weapons[WeaponID].Nearest)
		{
			...
			if (m_nBestBone == -1)
			{
				...
				for (int j = 0; j < (nt == 0 ? 8 : nt == 1 ? 14 : 20); j++)
				{
					...
					if (fov < bestFov)
					{
						if (!U::IsVisible(vSrc, vec, pLocalEntity, pBaseEntity, Options::Aimbot::SmokeCheck))
							continue;
	...
	void Run(CUserCmd* cmd, bool* &bSendPacket)
	{
		...
		if (canSilent)
			canSilent = (1 + (rand() % 101)) <= weapons[WeaponID].pSilentHitchance;
		...
		if (cmd->buttons & IN_ATTACK || Options::Aimbot::AutoFire)
		{
			...
			if (Options::Aimbot::KillDelay  && !Options::Aimbot::AutoFire && (G::KillDelayEnd || Interface.Globals->curtime <= G::KillDelayTime))
				return;
			...
			if (Options::Aimbot::AutoFire) cmd->buttons |= IN_ATTACK;
			...
			if (Options::Aimbot::AimType == 0 ||
				Options::Aimbot::AimType == 1)
				dst = Smooth(src, dst, smooth);
			else if (Options::Aimbot::AimType == 2)
