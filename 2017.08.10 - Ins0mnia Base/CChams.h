...
namespace CChams
{
	...
	{
		static RainbowColor handsColor = RainbowColor();

		if (pInfo.pModel)
		{
			std::string szModelName = Interface.ModelInfo->GetModelName(pInfo.pModel);
			if (Options::Visuals::Chams::Enabled && szModelName.find("player") != std::string::npos)
			{
				...
				if (pBaseEntity)
				{
					...
					if (valid)
					{
						if (Options::Visuals::Chams::Style == 0 ||
							Options::Visuals::Chams::Style == 1)
						{
			...
			else if (Options::Visuals::Hands::Enabled && szModelName.find(XorStr("arms")) != std::string::npos)
			{
				...
				if (Options::Visuals::Hands::Style == 0)
				{
