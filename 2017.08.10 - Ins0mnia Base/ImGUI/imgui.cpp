...
void ImGui::Render()
{
    ...
    if (g.Style.Alpha > 0.0f)
    {
        ...
        if (g.IO.MouseDrawCursor)
        {
            ...
            g.OverlayDrawList.AddImage(tex_id, pos+ImVec2(1,0), pos+ImVec2(1,0) + size, cursor_data.TexUvMin[1], cursor_data.TexUvMax[1], IM_COL32(0,0,0,0));        // Shadow
            g.OverlayDrawList.AddImage(tex_id, pos+ImVec2(2,0), pos+ImVec2(2,0) + size, cursor_data.TexUvMin[1], cursor_data.TexUvMax[1], IM_COL32(0,0,0,0));        // Shadow
            g.OverlayDrawList.AddImage(tex_id, pos,             pos + size,             cursor_data.TexUvMin[1], cursor_data.TexUvMax[1], IM_COL32(0,0,0,0));       // Black border
...
void ImGui::RenderTextClipped(const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect, bool tab, bool selected)
{
    // Hide anything after a '##' string
    ...
    if (need_clipping)
    {
        ...
        window->DrawList->AddText(g.Font, g.FontSize, pos, GetColorU32(tab ? selected ? ImGuiCol_TabTextActive : ImGuiCol_TabText : ImGuiCol_Text), text, text_display_end, 0.0f, &fine_clip_rect);
    }
    else
    {
        window->DrawList->AddText(g.Font, g.FontSize, pos, GetColorU32(tab ? selected ? ImGuiCol_TabTextActive : ImGuiCol_TabText : ImGuiCol_Text), text, text_display_end, 0.0f, NULL);
    }
    if (g.LogEnabled)
        LogRenderedText(pos, text, text_display_end);
}
...
ImVec2 ImGui::CalcTextSize(const char* text, const char* text_end, bool hide_text_after_double_hash, float wrap_width, bool tab)
{
    ImGuiContext& g = *GImGui;
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            if (flags & ImGuiWindowFlags_ShowBorders)
            {
                window->DrawList->AddRect(window->Pos+ImVec2(1,1), window->Pos+window->Size+ImVec2(1,1), GetColorU32(ImGuiCol_BorderShadow), window_rounding, -1, 2);
                window->DrawList->AddRect(window->Pos, window->Pos+window->Size, GetColorU32(ImGuiCol_Border), window_rounding, -1, 2);
                if (!(flags & ImGuiWindowFlags_NoTitleBar))
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, bool tab, bool selected)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true, -1.0F, true);
	...
	ImU32 col;
	if (tab)
		col = GetColorU32(selected ? ImGuiCol_TabSelected : ((hovered && held) ? ImGuiCol_TabActive : hovered ? ImGuiCol_TabHovered : ImGuiCol_Tab));
	else
		col = GetColorU32((hovered && held) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
	//if (tab) RenderFrame(ImVec2(bb.Min.x - 3, bb.Min.y - 3), ImVec2(bb.Max.x + 3, bb.Max.y + 3), GetColorU32(ImVec4(0, 0, 0, 1)), true, style.FrameRounding);
	RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb, tab, selected);
...
bool ImGui::Tab(const char* label, const ImVec2& size_arg, bool selected)
{
	return ButtonEx(label, size_arg, 0, true, selected);
	/*int flags = 0;
    ...
    float spacing_L = (float)(int)(style.ItemSpacing.x);
    float spacing_U = (float)(int)(style.ItemSpacing.y);
    ...
    if (hovered || selected)
    {
        const ImU32 col = GetColorU32(selected ? ImGuiCol_TabSelected : (held && hovered) ? ImGuiCol_TabActive : hovered ? ImGuiCol_TabHovered : ImGuiCol_Tab);
        RenderFrame(bb_with_spacing.Min, bb_with_spacing.Max, col, false, 0.0f);
		if (selected)
		{
			RenderFrame(ImVec2(bb_with_spacing.Min.x, bb_with_spacing.Max.y - 2), bb_with_spacing.Max, GetColorU32(ImGuiCol_SelectedLine), false);
		}
    }
    ...
    RenderTextClipped(bb.Min, bb_with_spacing.Max, label, NULL, &label_size, ImVec2(0.5f,0.5f));
	...
	//return Selectable(label, selected, 0, size_arg);
	//return ButtonEx(label, size_arg, 0, true, selected);
}
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags, ImVec4 color)
{
	...
	char value_buf[64];
	const char* value_buf_end = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), "%.3f", *v);
...
void ImGui::SliderColor3(const char* label, float v[3])
{
	int tempR = (int)(255 * v[0]);
	int tempG = (int)(255 * v[1]);
	int tempB = (int)(255 * v[2]);
	ImGui::Text(label);
	ImGui::PushID(label);
	ImGui::SliderInt("R", &tempR, 0, 255);
	ImGui::SliderInt("G", &tempG, 0, 255);
	ImGui::SliderInt("B", &tempB, 0, 255);

	v[0] = (float)(tempR / 255.f);
	v[1] = (float)(tempG / 255.f);
	v[2] = (float)(tempB / 255.f);
	ImGui::PopID();
	ImGui::NewLine();
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    {
		//window->DrawList->AddRectFilled(check_bb.Min+ImVec2(pad,pad), check_bb.Max-ImVec2(pad,pad), GetColorU32(ImGuiCol_CheckMark), style.FrameRounding);
		ImVec2* vecs = new ImVec2[3];
		vecs[0] = check_bb.Min + ImVec2(5, check_sz2 + 1);
		vecs[1] = check_bb.Max - ImVec2(check_sz2 + 1, 5);
		vecs[2] = check_bb.Max - ImVec2(3, check_sz2 + 3);
		window->DrawList->AddPolyline(vecs, 3, IM_COL32(220, 220, 220, 255), false, 1.5f, true);
    }
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImVec2 size, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	IM_ASSERT(!(flags & ImGuiInputTextFlags_Multiline)); // call InputTextMultiline()
	return InputTextEx(label, buf, (int)buf_size, size, flags, callback, user_data);
}
...
bool ImGui::Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items, ImVec2 size)
{
    int items_count = 0;
    ...
    bool value_changed = Combo(label, current_item, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, height_in_items, size);
    return value_changed;
}

// Combo box function.
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items, ImVec2 size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
bool ImGui::ColorEdit3(const char* label, float col[3], bool preview)
{
    float col4[4];
    col4[0] = col[0];
    col4[1] = col[1];
    col4[2] = col[2];
    col4[3] = 1.0f;
    const bool value_changed = ColorEdit4(label, col4, false, false);
    col[0] = col4[0];
    col[1] = col4[1];
    col[2] = col4[2];
    return value_changed;
...
bool ImGui::ColorEdit4(const char* label, float col[4], bool alpha, bool preview)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    case ImGuiColorEditMode_HSV:
        {
            ...
            const bool hide_prefix = true;// (w_item_one <= CalcTextSize("M:999").x);
    ...
    if (preview && ColorButton(col_display))
...
void ImGui::Separator(float spacing_y, float line_width)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    const ImRect bb(ImVec2(x1, window->DC.CursorPos.y + spacing_y), ImVec2(x2, window->DC.CursorPos.y + spacing_y  + 1.0f));
    ...
    window->DrawList->AddLine(bb.Min, ImVec2(bb.Max.x,bb.Min.y), GetColorU32(ImGuiCol_Border), line_width);
...
void ImGui::NewLine(float spacing_y)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    if (window->DC.CurrentLineHeight > 0.0f)     // In the event that we are on a line with items that is smaller that FontSize high, we will preserve its height.
        ItemSize(ImVec2(0,0));
    else if (spacing_y == 0.0f)
        ItemSize(ImVec2(0.0f, GImGui->FontSize));
	else
		ItemSize(ImVec2(0.0f, spacing_y));
}
