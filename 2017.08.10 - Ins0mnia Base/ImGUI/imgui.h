...
struct ImVec4
{
	...
	bool Empty() { return (x == 0 && y == 0 && z == 0 && w == 0); }
...
namespace ImGui
{
    ...
    // Cursor / Layout
	IMGUI_API void          Separator(float spacing_y = 0.0f, float line_width = 1.0f);                                                        // horizontal line
    IMGUI_API void          SameLine(float pos_x = 0.0f, float spacing_w = -1.0f);              // call between widgets or groups to layout them horizontally
    IMGUI_API void          NewLine(float spacing_y = 0.0f);                                                          // undo a SameLine()
	...
	IMGUI_API bool          Tab(const char* label, const ImVec2& size = ImVec2(0, 0), bool selected = false); // tab
    ...
    IMGUI_API bool          Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items = -1, ImVec2 size = ImVec2(0, 0));      // separate items with \0, end item-list with \0\0
	IMGUI_API bool			Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**),	void* data, int items_count, int height_in_items = -1, ImVec2 size = ImVec2(0, 0));
    IMGUI_API bool          ColorButton(const ImVec4& col, bool small_height = false, bool outline_border = true);
    IMGUI_API bool          ColorEdit3(const char* label, float col[3], bool preview = true);                            // Hint: 'float col[3]' function argument is same as 'float* col'. You can pass address of first element out of a contiguous set, e.g. &myvector.x
    IMGUI_API bool          ColorEdit4(const char* label, float col[4], bool show_alpha = true, bool preview = true);    // "
	...
	IMGUI_API bool          InputText(const char* label, char* buf, size_t buf_size, ImVec2 size, ImGuiInputTextFlags flags = 0, ImGuiTextEditCallback callback = NULL, void* user_data = NULL);
	...
	IMGUI_API void			SliderColor3(const char* label, float v[3]);
    IMGUI_API bool          SliderFloat(const char* text, float* v, float v_min, float v_max, const char* display_format = "%.1f", float power = 1.0f, ImVec2 size = ImVec2(0, 0), ImVec4 color = ImVec4(0, 0, 0, 0));     // adjust display_format to decorate the value with a prefix or a suffix. Use power!=1.0 for logarithmic sliders
    ...
    IMGUI_API ImVec2        CalcTextSize(const char* text, const char* text_end = NULL, bool hide_text_after_double_hash = false, float wrap_width = -1.0f, bool tab = false);
